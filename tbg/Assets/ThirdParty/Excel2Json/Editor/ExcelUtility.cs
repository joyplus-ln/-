...
public class ExcelUtility
{
	...
	/// <param name="JsonPath">Json文件路径</param>
	/// <param name="Header">表头行数</param>
	public void ConvertToCharacter(string JsonPath, Encoding encoding, CharacterItem item)
    {
        //判断Excel文件中是否存在数据表
        if (m_ResultSet.Tables.Count < 1)
            return;
        ...
        for (int i = 3; i < rowCount; i++)
        {
            ...
            {
                //读取第1行数据作为表头字段
                string field = mSheet.Rows[2][j].ToString().ToLower();
                //Key-Value对应
                row[field] = mSheet.Rows[i][j];
            }
            ...
            string json = JsonConvert.SerializeObject(table, Newtonsoft.Json.Formatting.Indented);
            //写入文件
            //CharacterItem characterItem = ScriptableObject.Instantiate(item);
            //CharacterUtils.MakeCharacterItem(row, characterItem);
            //string fullPath = Path.Combine(Path.Combine("Assets", "Excel"), characterItem.title + ".asset");
            //AssetDatabase.CreateAsset(characterItem, fullPath);
        }


    }

    public void ConvertToEquipment(string JsonPath, Encoding encoding, EquipmentItem item)
    {
        //判断Excel文件中是否存在数据表
        if (m_ResultSet.Tables.Count < 1)
            return;
        ...
        for (int i = 3; i < rowCount; i++)
        {
            ...
            string json = JsonConvert.SerializeObject(table, Newtonsoft.Json.Formatting.Indented);
            //写入文件
            //EquipmentItem characterItem = ScriptableObject.Instantiate(item);
            //EquipUtils.MakeEquipmentItem(row, characterItem);
            //string fullPath = Path.Combine(Path.Combine("Assets", "Excel"), characterItem.title + ".asset");
            //AssetDatabase.CreateAsset(characterItem, fullPath);
        }
    ...
    public void ConvertToSkill(string JsonPath, Encoding encoding, CustomSkill item)
    {
        ////判断Excel文件中是否存在数据表
        //if (m_ResultSet.Tables.Count < 1)
        //    return;
        ...
        //    string json = JsonConvert.SerializeObject(table, Newtonsoft.Json.Formatting.Indented);
        //    //写入文件
        //    Skill characterItem = ScriptableObject.Instantiate(item);
        //    SkillUtils.MakeEquipmentItem(row, characterItem);
        //    string fullPath = Path.Combine(Path.Combine("Assets", "Excel"), characterItem.title + ".asset");
        //    AssetDatabase.CreateAsset(characterItem, fullPath);
        //}
