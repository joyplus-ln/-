...
public class UIFormationManager : UIBase
{
    ...
    public void Setup()
    {
        ...
        {
            var list = PlayerItem.characterDataMap.Values.Where(a => a.PlayerId == playerId && a.CharacterData != null).ToList();
            var elist = PlayerItem.equipDataMap.Values.Where(a => a.PlayerId == playerId && a.CharacterData != null).ToList();
            list.AddRange(elist);
            list.SortLevel();
